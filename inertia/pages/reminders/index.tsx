...
export default function Index() {
  const { reminders: remindersData, user, csrfToken } = usePage<PageProps>().props
  ...
  const toggleForm = () => {
    if (isFormVisible && (isEditing || editingReminderId)) {
      closeForm()
    } else {
    ...
    <>
      ...
      <div className="min-h-screen bg-[#1C1C1E] text-white">
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    handleSort(field, remindersData.meta.current_page)
                  }}
        ...
        {isFormVisible && (
          ...
          >
            <ReminderForm
              isEditing={isEditing}
              editingReminder={editingReminderId ? remindersData.data.find(b => b.id === editingReminderId) : undefined}
              onCancel={closeForm}
              onSuccess={closeForm}
            />
          </motion.div>
        ...
        <div className="px-6 py-6">
          ...
          {/* Pagination */}
          {remindersData.meta.last_page > 1 && (
            <motion.div
            ...
            >
              {/* Previous Button */}
              <button
                onClick={() => handlePageChange(remindersData.meta.current_page - 1, remindersData)}
                disabled={remindersData.meta.current_page === 1}
              ...
              <div className="px-4 py-2 bg-[#2C2C2E] rounded-lg text-sm">
                Page {remindersData.meta.current_page} of {remindersData.meta.last_page}
              </div>

              {/* Next Button */}
              <button
                onClick={() => handlePageChange(remindersData.meta.current_page + 1, remindersData)}
                disabled={remindersData.meta.current_page === remindersData.meta.last_page}
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#2C2C2E] text-white disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#3A3A3C] transition-colors duration-200"
              >
                Next
          ...
          {remindersData.data.length === 0 && (
            <div className="text-center py-12">
              <div className="text-[#98989D] text-lg mb-4">
                {filterFavorites ? 'No favorite reminders found' : 'No reminders found'}
              </div>
              <button
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          ...
          >
            <h3 className="text-lg font-semibold mb-4 text-white">Delete Reminder</h3>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                onClick={() => handleReminderDelete(deleteConfirm)}
